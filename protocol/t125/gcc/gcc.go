...
/*-----------------------------------------------------------------*/
func recoverCertificate(data []byte) (*x509.Certificate, error) {
    block, _ := pem.Decode(data)
    if block == nil {
        glog.Error("Error: Cannot decode certificate, skipping.")
        return nil, fmt.Errorf("failed to decode certificate")
    }

    cert, err := x509.ParseCertificate(block.Bytes)
    if err != nil {
        glog.Error("Error: Certificate is invalid, returning a dummy certificate.")
        return generateFallbackCertificate(), nil
    }
    return cert, nil
}

func generateFallbackCertificate() *x509.Certificate {
    privKey, _ := rsa.GenerateKey(rand.Reader, 2048)

    return &x509.Certificate{
        SerialNumber: big.NewInt(1),
        Subject:      pkix.Name{CommonName: "Fallback Certificate"},
        PublicKey:    &privKey.PublicKey,
    }
}

func generateFallbackRSAKey() *rsa.PublicKey {
    privKey, _ := rsa.GenerateKey(rand.Reader, 2048)
...
func (x *X509CertificateChain) GetPublicKey() (*rsa.PublicKey, error) {
	...
	glog.Info(x)
    if err != nil {
        glog.Info("Warning: Failed to parse certificate, trying to recover...")
        recoveredCert, err := recoverCertificate(data)
        if err != nil {
            return generateFallbackRSAKey(), nil
        }
        return recoveredCert.PublicKey.(*rsa.PublicKey), nil
    }
